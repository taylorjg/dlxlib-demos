...
> = ({ puzzle, solutionInternalRows, drawingOptions }) => {
  ...
  if (drawingOptions.showClues) {
    const maxNumRunGroupsHorizontally = max(
      puzzle.horizontalRunGroups.map((rg) => rg.lengths.length)
    );
    const maxNumRunGroupsVertically = max(
      puzzle.verticalRunGroups.map((rg) => rg.lengths.length)
    );
    numMarginSquares = Math.max(
      maxNumRunGroupsHorizontally,
      maxNumRunGroupsVertically
    );
  }

  const sizeWithMargin = numMarginSquares + puzzle.size;
  const SQUARE_WIDTH =
    (VIEWBOX_WIDTH - GRID_LINE_FULL_THICKNESS) / sizeWithMargin;
  const SQUARE_HEIGHT =
    (VIEWBOX_HEIGHT - GRID_LINE_FULL_THICKNESS) / sizeWithMargin;

  const calculateX = (col: number) =>
    (numMarginSquares + col) * SQUARE_WIDTH + GRID_LINE_HALF_THICKNESS;
  const calculateY = (row: number) =>
    (numMarginSquares + row) * SQUARE_HEIGHT + GRID_LINE_HALF_THICKNESS;
