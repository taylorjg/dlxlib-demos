...
const checkSolution = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  ...
  const totalRunGroupsCount = horizontalRunGroups.length + verticalRunGroups.length
...
const checkAllRunGroupsArePresent = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  for (const horizontalRunGroup of puzzle.horizontalRunGroups) {
    const internalRow = internalRows.find(({ runGroup }) => runGroup === horizontalRunGroup)
    expect(internalRow).toBeDefined()
  }

  for (const verticalRunGroup of puzzle.verticalRunGroups) {
    const internalRow = internalRows.find(({ runGroup }) => runGroup === verticalRunGroup)
    expect(internalRow).toBeDefined()
  }
}

const checkHorizontalRunGroupsAreCorrect = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  ...
  const rebuildHorizontalRunGroupLengths = (row: number): number[] => {
    const cols = internalRows
      .filter(({ runGroup }) =>
        runGroup.runGroupType === RunGroupType.Vertical)
      .flatMap(internalRow =>
  ...
  for (const horizontalRunGroup of puzzle.horizontalRunGroups) {
    const runGroupLengths = rebuildHorizontalRunGroupLengths(horizontalRunGroup.row)
    expect(horizontalRunGroup.lengths).toEqual(runGroupLengths)
  }
}

const checkVerticalRunGroupsAreCorrect = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  ...
  const rebuildVerticalRunGroupLengths = (col: number): number[] => {
    const rows = internalRows
      .filter(({ runGroup }) =>
        runGroup.runGroupType === RunGroupType.Horizontal)
      .flatMap(internalRow =>
        internalRow.coordsLists.flatMap(coordsList =>
  ...
  for (const verticalRunGroup of puzzle.verticalRunGroups) {
    const runGroupLengths = rebuildVerticalRunGroupLengths(verticalRunGroup.col)
