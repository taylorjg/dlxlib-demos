...
export const Drawing: React.FunctionComponent<LocalDrawingProps> = ({
    ...
    return rows.map((row) => {
      ...
      return (
        <line
          ...
          y2={y}
          strokeWidth={
            isThickLine ? GRID_LINE_FULL_THICKNESS : GRID_LINE_HALF_THICKNESS
          }
  ...
  const drawInitiaValues = (): JSX.Element[] => {
    return puzzle.initialValues.map((initialValue) =>
      drawValue(initialValue.coords, initialValue.value, true)
    );
  };
  ...
  const drawCalculatedValues = (): JSX.Element[] => {
    return solutionInternalRows
      ...
      .map((internalRow) =>
        drawValue(internalRow.coords, internalRow.value, false)
      );
  };
