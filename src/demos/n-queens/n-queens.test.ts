...
const checkRows = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  const distinctRows = distinct(
    internalRows.map((internalRow) => internalRow.coords.row)
  );
  expect(distinctRows).toHaveLength(puzzle.size);
};

const checkCols = (puzzle: Puzzle, internalRows: InternalRow[]): void => {
  const distinctCols = distinct(
    internalRows.map((internalRow) => internalRow.coords.col)
  );
  expect(distinctCols).toHaveLength(puzzle.size);
};

const checkDiagonalsUpperLeftToLowerRight = (
  puzzle: Puzzle,
  ...
  for (const index of range(diagonalColumnCount)) {
    ...
    const count = diagonalCoords.filter((coords) =>
      internalRows.some((internalRow) => sameCoords(internalRow.coords, coords))
    ).length;
...
const checkDiagonalsUpperRightToLowerLeft = (
  puzzle: Puzzle,
  internalRows: InternalRow[]
): void => {
  const { size } = puzzle;
